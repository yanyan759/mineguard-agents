...
def test_g2_real_docs_index() -> None:
    ...
    if ok_dir:
        for sub in ("coal_mine_accident_cases", "coal_mine_safety_regulations", "coal_mine_support_schemes"):
            ...
            files = [f for f in files if f.is_file() and f.name not in {"README.md", ".gitkeep"}]
            check(f"G2 {sub} 可选资料目录就绪", sub_path.is_dir(), f"{len(files)} 份外部资料已挂载")
    ...
    check("G2 _index_real_docs 空目录安全",
          isinstance(index, list) and all(d.get("file_count", 0) > 0 for d in index),
          f"{domains}（没有外部资料时允许为空）")
