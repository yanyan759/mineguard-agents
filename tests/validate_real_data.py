...
当部署方通过外部路径提供真实案例时，验证其进入 Neo4j 图谱和知识检索链路；
默认公开源码不携带该资料，缺少外部路径时脚本以 SKIP 结束，不影响基础发布验收。
...
def main() -> int:
    if not REAL_CASES_YAML.is_file():
        print("[SKIP] 未配置 MINEGUARD_REAL_CASES_PATH，公开源码不内置真实案例文件。")
        print("       配置外部 YAML 后再次运行本脚本，以执行真实数据域验收。")
        return 0
    print("=" * 60)
