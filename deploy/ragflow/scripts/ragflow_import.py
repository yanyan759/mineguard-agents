...
流程：建 3 个知识库（dataset）→ 上传 prepare_real_docs.py 归类的真实资料
...
DOCS_DIR = AGENT1_ROOT / "deploy" / "ragflow" / "docs" / "real"
...
def _validate_docs_tree() -> None:
    ...
    for dataset, folder in DATASETS.items():
        ...
        files = sorted(f for f in target.iterdir() if f.is_file()) if target.exists() else []
        print(f"  {dataset} ← {folder}/（{len(files)} 篇）")
        if not files:
            print("    ⚠️ 无文档，请先运行 prepare_real_docs.py")
...
def _upload_docs(dataset_id: str, folder: str) -> list[str]:
    ...
    target = DOCS_DIR / folder
    for file in sorted(f for f in target.iterdir() if f.is_file()):
