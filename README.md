...
├── scripts/                     # 历史回放和浏览器验证
├── docs/                        # 方案、契约和验证记录
├── 煤矿资料收集/                  # 公开来源索引；原始语料按需获取，不随仓库发布
├── 真实数据投喂/                  # 脱敏模板、字段说明与本地整理工具
...
| 上传资源文件仍待输入 | 在矿方配置框选择资源台账类别上传 JSON/CSV，勾选资料并保存配置；核对任务测区、库存时间，以及本次 resources 是否覆盖了台账 |
...
源码仓库不包含默认语料。全新用户只需安装代码依赖、启动 Redis、后端和前端，即可使用七个演示场景或 README 中的示例输入得到六 Agent 结果；可选语料不会成为默认启动前置条件。
...
- [资料准备与导入规范](docs/真实资料爬取与整理要求清单.md)
- [公开资料来源与导入说明](docs/资料爬取任务书.md)
- [资料导入模板与示例](真实数据投喂/README.md)
...
维护者可查阅 [实际数据链路验证记录](docs/actual-data-verification-2026-09-16.md)，了解测试输入、覆盖范围与历史验证结果。
