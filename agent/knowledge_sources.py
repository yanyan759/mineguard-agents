...
# 真实资料库根目录（RAGFlow 导入源，deploy/ragflow/docs/real，160 份真实 HTML/PDF）
# 确定性双轨通过 _index_real_docs() 感知其存在与规模（结构索引，不解析正文）。
_REAL_DOCS_DIR = (
    Path(__file__).resolve().parent.parent / "deploy" / "ragflow" / "docs" / "real"
)
# 数据域 → 真实资料子目录映射
_REAL_DOCS_DOMAINS = {
...
def _index_real_docs() -> list[dict]:
    """扫描真实资料库（deploy/ragflow/docs/real）三大域，返回文件索引元数据。

    真实资料（160 份 HTML/PDF）由资料爬取任务导入 RAGFlow；本索引为确定性双轨
    提供对真实资料库的结构感知（域/文件数/文件清单），不解析正文、不改变检索语义。
    目录不存在或为空时返回空列表（结构存在，数据可后续补充）。
    """
...
def _yaml_backend(risk_level: RiskLevel) -> RetrievalData:
    ...
    if _real_cases_enabled():
        real_data = _load_yaml(_REAL_CASES_FILENAME)
